# WhisperTray

[Сайт](https://krou4.github.io/Whisper-Tray/) | [Русский](README.md) | [English](README.en.md)

<p align="center">
  <img src="assets/whispertray-logo.png" width="160" alt="Логотип WhisperTray">
</p>

WhisperTray — настольное приложение для диктовки под Windows, macOS и Linux.
Нажмите глобальное сочетание клавиш, произнесите текст — приложение распознает
речь и вставит результат в активное окно. Оно работает локально либо с Groq,
когда пользователь явно выбирает облачный режим и указывает свой API-ключ.

> Актуальный релиз: [WhisperTray 1.2.0](https://github.com/KROU4/Whisper-Tray/releases/latest).
> Нативные установщики собираются автоматически для Windows, macOS и Linux.

## Возможности

- Запись с выбранного микрофона: не более 10 минут за одну диктовку.
- Локальное распознавание через `faster-whisper` в режиме «Приватность».
- Распознавание через Groq в режиме «Скорость» с API-ключом пользователя.
- Индикаторы записи, обработки, результата и ошибки в окне, оверлее и трее.
- Независимые настройки запуска вместе с системой и запуска сразу в трее.
- Распознавание отдельных аудио- и видеофайлов.
- Необязательная локальная история расшифровок.
- Отмена обработки, повтор неудачной диктовки, уровень микрофона и таймер.
- Просмотр и поиск истории; копирование, сохранение и открытие результатов файлов.

После ошибки диктовки временная запись сохраняется локально не более пяти минут
для кнопки «Повторить». Она удаляется при отмене, следующей записи или выходе.
Обычные успешные записи удаляются сразу после обработки. История текста по
умолчанию отключена. Для файлов сохраняется отдельный TXT; ошибка сохранения
не удаляет полученный текст из окна.

«Проверить микрофон» записывает пять секунд для локального прослушивания.
Стадии обработки и загрузки модели видны в интерфейсе; проценты показываются
только при измеримом прогрессе. Для Groq действуют максимум два повтора,
60 секунд на запрос и общий бюджет облачной задачи 180 секунд.

## Режимы и приватность

| Режим | Куда попадает аудио | Резервный вариант |
| --- | --- | --- |
| Приватность | Не покидает устройство: используется локальная модель Whisper | Нет |
| Скорость | Отправляется в Groq | Локальная модель — только при явном включении в настройках |

Режим по умолчанию — «Приватность». Ошибка Groq не меняет режим и не
отправляет аудио другому сервису. API-ключ Groq хранится в системном хранилище
учетных данных, а не в `config.json`.

## Как получить ключ Groq

Ключ нужен только для режима «Скорость». Для локального режима
«Приватность» учетная запись Groq не требуется.

1. Войдите в [GroqCloud Console](https://console.groq.com/) или создайте
   учетную запись.
2. Откройте раздел [API Keys](https://console.groq.com/keys).
3. Нажмите **Create API Key**, укажите понятное имя, например
   `WhisperTray`, и скопируйте созданный ключ.
4. В WhisperTray выберите профиль «Скорость». Ключ можно ввести во время
   первого запуска либо позже через «Настройки» → «Основное» → «Ключ Groq
   API».
5. Нажмите «Проверить ключ», затем сохраните настройки.

Не записывайте ключ в `config.json`, не отправляйте его другим людям и не
публикуйте в GitHub. WhisperTray сохраняет ключ в системном хранилище учетных
данных. Если ключ случайно раскрыт, удалите его в GroqCloud Console и создайте
новый.

### Лимиты Groq

WhisperTray по умолчанию использует модель `whisper-large-v3-turbo`. Для нее
Groq указывает следующие базовые лимиты бесплатного плана:

| Ограничение | Бесплатный план |
| --- | ---: |
| Запросов в минуту | 20 |
| Запросов в день | 2 000 |
| Аудио в час | 7 200 секунд, то есть 2 часа |
| Аудио в день | 28 800 секунд, то есть 8 часов |
| Размер одного загружаемого файла | 25 МБ |

Срабатывает то ограничение, которое будет достигнуто первым. При превышении
лимита Groq отвечает ошибкой `429 Too Many Requests`; после сброса квоты можно
продолжить работу. Точные лимиты конкретной учетной записи всегда показаны на
странице [GroqCloud Limits](https://console.groq.com/settings/limits), поскольку
Groq может менять их независимо от выпуска WhisperTray. Публичные справочные
таблицы находятся в документации Groq: [Rate Limits](https://console.groq.com/docs/rate-limits)
и [Speech to Text](https://console.groq.com/docs/speech-to-text).

Собственное ограничение WhisperTray — не более 10 минут на одну диктовку. Оно
не увеличивает и не заменяет квоты Groq. Для платного использования официальная
цена `whisper-large-v3-turbo` на момент обновления README составляет $0,04 за
час обработанного аудио; лимит файла для Developer-плана — 100 МБ.

## Установка и первый запуск

Загрузите установщик для своей ОС из [GitHub Releases](https://github.com/KROU4/Whisper-Tray/releases):

- Windows — `.msi`;
- macOS — `.dmg` или `.pkg`;
- Linux — нативный пакет, собранный для Ubuntu в релизном workflow.

В Windows мастер установки позволяет выбрать папку, показывает лицензионное
соглашение, предлагает создать ярлык на рабочем столе и после установки может
сразу открыть главное окно WhisperTray.

Первый запуск состоит из трёх коротких шагов: выбор локального или облачного
режима, подготовка выбранного способа распознавания и настройка микрофона с
горячей клавишей. Затем:

1. В режиме «Приватность» подготовьте или скачайте локальную модель в
   настройках.
2. В режиме «Скорость» введите и проверьте ключ Groq. Ключ сохраняется только
   после успешной записи в системное хранилище.
3. Чтобы сменить сочетание, откройте «Настройки» → «Основное», нажмите
   «Изменить» рядом с горячей клавишей и просто нажмите новое сочетание.

Локальные модели не входят в установщик и загружаются по запросу. Их размер в
приложении — примерно от 75 МБ (`tiny`) до 2,9 ГБ (`large`).

### Разрешения и ограничения

- Windows: разрешите доступ к микрофону в параметрах конфиденциальности.
- macOS: разрешите доступ к микрофону и к разделу «Универсальный доступ»;
  второе разрешение нужно для глобального сочетания и вставки текста.
- Linux: микрофон должен быть доступен в пользовательском сеансе. Глобальные
  сочетания работают в X11; в Wayland приложение явно сообщает, что функция
  недоступна.

## Где хранятся данные

| Система | Каталог приложения |
| --- | --- |
| Windows | `%LOCALAPPDATA%\\WhisperTray` |
| macOS | `~/Library/Application Support/WhisperTray` |
| Linux | `$XDG_DATA_HOME/WhisperTray` или `~/.local/share/WhisperTray` |

В этом каталоге находятся настройки, технические логи, необязательная история
и результаты распознавания файлов. Логи не содержат аудио, текст расшифровок
или учетные данные.

## Разработка

Нужен Python 3.10 или новее. В PowerShell:

```powershell
python -m venv .venv
.\.venv\Scripts\Activate.ps1
python -m pip install -r requirements-dev.txt
python main.py
```

На macOS и Linux активируйте окружение командой `source .venv/bin/activate`.
Проверьте изменения:

```powershell
python -m pytest -q
ruff check .
python -m compileall -q .
```

`requirements.txt` и `requirements-dev.txt` — зафиксированные файлы зависимостей,
сгенерированные из соответствующих `.in` файлов через `uv pip compile`.

## Упаковка и релиз

Для нативных пакетов используется Briefcase: каждый пакет нужно собирать на
своей целевой ОС. Workflow `.github/workflows/release.yml` срабатывает при
отправке тега `v*`, тестирует проект на Windows, macOS и Ubuntu, собирает
нативные пакеты и создаёт черновик GitHub Release. Публикация выполняется после проверки пакетов. Локальная команда сборки:

```powershell
python -m briefcase create windows --no-input
python -m briefcase build windows --no-input
python -m briefcase package windows --no-input
```

Для macOS или Linux замените `windows` на `macOS` или `linux` и выполняйте
команду на соответствующей операционной системе.

## Безопасность

Не добавляйте в Git `config.json`, логи, аудиофайлы, историю расшифровок или
API-ключи. Если ключ попал в историю Git, немедленно отзовите его. Правила для
сообщений об уязвимостях описаны в [политике безопасности](.github/SECURITY.md).
