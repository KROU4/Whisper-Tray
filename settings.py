...
def open_settings(root: tk.Tk, state):
    ...
    # ---- Строка 0: модель для real-time записи ----
    tk.Label(win, text='Модель (запись):', anchor='w').grid(
    ...
    # ---- Строка 1: модель для транскрипции файлов ----
    tk.Label(win, text='Модель (файлы):', anchor='w').grid(
        row=1, column=0, sticky='w', **pad
    )
    file_model_var = tk.StringVar(value=_label_for(cfg.get('file_model', 'large')))
    ttk.Combobox(
        win, textvariable=file_model_var,
        values=MODEL_LABELS, state='readonly', width=20
    ...
    hotkey_frame.grid(row=3, column=1, sticky='w', **pad)
    ...
    def save():
        ...
        new_file_model = _name_for(file_model_var.get())
        ...
        old_file_model = state.config.get('file_model', 'large')
        state.config['model']        = new_model
        state.config['file_model']   = new_file_model
        ...
        # Сбрасываем модель real-time если изменился её размер
        ...
        # Сбрасываем модель файлового транскрибатора если изменился её размер
        ft = getattr(state, 'file_transcriber', None)
        if ft and new_file_model != old_file_model:
            ft.reload(new_file_model)
